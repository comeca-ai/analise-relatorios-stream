def criar_visualizacao(vis_info):
    ...
    try:
        ...
        # Tentar criar gráfico básico (barras é o mais seguro)
        try:
            # Para garantir que as colunas numéricas são tratadas corretamente
            ...
            # Criar gráfico de barras simples (mais estável)
            fig = px.bar(
                df, 
                x=df.columns[0], 
                y=df.columns[1] if len(df.columns) > 1 else [1] * len(df),
                title=titulo
            )
            ...
            st.write("Visualizando os dados em formato de tabela:")
            st.dataframe(df)
        
        st.markdown("</div>", unsafe_allow_html=True)
    except Exception as e:
        st.error(f"Erro geral na criação da visualização: {str(e)}")


# === INTERFACE PRINCIPAL ===
def main():
    """
    Função principal que organiza a interface do aplicativo
    """
    try:
        ...
        **Tipos de análise:**
        ...
        if uploaded_file and openai_api_key:
            try:
                ...
                st.markdown("<h2 class='sub-header'>Visualizações Extraídas</h2>", unsafe_allow_html=True)
                        ...
                        if len(st.session_state.visualizacoes_por_bloco) <= i or st.session_state.visualizacoes_por_bloco[i] is None:
                            with st.spinner(f"Analisando bloco {i+1} e gerando visualizações..."):
                                ...
                                try:
                                    ...
                                    st.error(f"Erro ao processar bloco {i+1}: {str(e)}")
                                    visualizacoes = []
                        else:
                        ...
                        if not visualizacoes:
                            ...
                            if st.button(f"Extrair dados manualmente do Bloco {i+1}", key=f"manual_extract_{i}"):
                                try:
                                    ...
                                    st.error(f"Erro ao extrair dados manualmente: {str(e)}")
                ...
                st.warning("Houve um problema ao processar o arquivo. Tente novamente ou entre em contato com o suporte.")
        else:
        ...
        st.warning("A aplicação encontrou um erro crítico. Por favor, recarregue a página ou contate o suporte.")
...
if __name__ == "__main__":
    try:
        ...
        st.error(f"Erro crítico na aplicação: {str(e)}")
        st.warning("A aplicação encontrou um erro crítico. Por favor, recarregue a página ou contate o suporte.")def extrair_visualizacoes_do_texto(texto: str):
    """
...
def main():
    ...
    st.markdown("<h1 class='main-header'>Analisador Estratégico de Mercado com IA</h1>", unsafe_allow_html=True)
    st.markdown("""
    Este aplicativo analiza documentos e extrai dados para criar visualizações gráficas automaticamente.
    Carregue um documento como um relatório de mercado, análise de setor ou estudo competitivo, e o aplicativo gerará
    gráficos interativos com base nos dados encontrados.
    """)
...
, '')
        ...
        st.error(f"Erro ao extrair visualizações: {str(e)}")
        return []
...
def criar_visualizacao_sintetica(texto):
    ...
    try:
        ...
        for num in numeros:
            # Remove símbolos e converte vírgula para ponto
            num_clean = num.replace('Rimport streamlit as st
import openai
import docx
import re
import io
...
, '').replace('%', '').replace(',', '.')
        ...
        st.error(f"Erro ao criar visualização sintética: {str(e)}")
        return []
...
def processar_bloco_com_fallback(bloco, tipo_analise):
    ...
    try:
        ...
        st.error(f"Erro ao processar bloco: {str(e)}")
        return []
        ...
        if tipo_dados == "Tabela":
            ...
            if st.button("Processar tabela"):
                try:
                    ...
                    st.error(f"Não foi possível interpretar os dados como tabela: {str(e)}")
            ...
            if st.button("Processar lista"):
                try:
                    ...
                    st.error(f"Erro ao processar lista: {str(e)}")
                ...
                try:
                    ...
                    for linha in linhas:
                        if ':' in linha:
                            ...
                            # Tenta converter para número
                            try:
                                val_clean = val.strip().replace('%', '').replace('Rimport streamlit as st
import openai
import docx
import re
import io
...
, '').replace(',', '.')
            ...
            if st.button("Processar dados personalizados"):
                try:
                    ...
                    st.error(f"Não foi possível processar os dados: {str(e)}")
        
        return None
    except Exception as e:
        st.error(f"Erro na interface de extração manual: {str(e)}")
